...
def calculate_sample_position(offset_horizontal, offset_vertical, lowest):
    h_over_width = CameraConfig.horizontalFOV / (CameraConfig.halfImageWidth * 2)
    v_over_height = CameraConfig.verticalFOV / (CameraConfig.halfImageHeight * 2)
...
def error_function(params, data):
    offset_horizontal, offset_vertical = params
    total_error = 0

    for sample in data:
        calc_x, calc_y = calculate_sample_position(offset_horizontal, offset_vertical, sample["lowest"])
...
if optimization_type == "local":
    ...
    initial_params = np.array([CameraConfig.offsetHorizontal, CameraConfig.offsetVertical], dtype=float)
...
optimal_offsetHorizontal, optimal_offsetVertical = result.x
